...
from .data_entry import mmlu_formatter
...
def load_MMLU_X_Y(phase, model_name, with_entropy=True, MMLU_TASKS="all"):
    ...
    for task_idx, task in enumerate(task_list):
        ...
        other1_query_answer_ave_mid_new = torch.cat(
            (other1_query_average_mid_layer_new, other1_answerm_mid_layer_new),
            dim=1,
        )
        ...
        other2_query_answer_ave_mid_new = torch.cat(
            (other2_query_average_mid_layer_new, other2_answerm_mid_layer_new),
            dim=1,
        )
        ...
        if with_entropy:
            ...
            other1_query_average_mid_layer_new = torch.cat(
                (other1_query_average_mid_layer_new, other1_entropy_features),
                dim=1,
            )
            other1_query_average_last_layer_new = torch.cat(
                (other1_query_average_last_layer_new, other1_entropy_features),
                dim=1,
            )
            ...
            other1_answerm_mid_layer_new = torch.cat(
                (other1_answerm_mid_layer_new, other1_entropy_features), dim=1
            )
            other1_answerm_last_layer_new = torch.cat(
                (other1_answerm_last_layer_new, other1_entropy_features), dim=1
            )
            other1_query_answer_ave_mid_new = torch.cat(
            ...
            other2_query_average_mid_layer_new = torch.cat(
                (other2_query_average_mid_layer_new, other2_entropy_features),
                dim=1,
            )
            other2_query_average_last_layer_new = torch.cat(
                (other2_query_average_last_layer_new, other2_entropy_features),
                dim=1,
            )
            ...
            other2_answerm_mid_layer_new = torch.cat(
                (other2_answerm_mid_layer_new, other2_entropy_features), dim=1
            )
            other2_answerm_last_layer_new = torch.cat(
                (other2_answerm_last_layer_new, other2_entropy_features), dim=1
            )
            other2_query_answer_ave_mid_new = torch.cat(
        ...
        if task_idx == 0:
            ...
            other1_answerm_mid_layer = torch.cat(
                (other1_answerm_mid_layer, other1_answerm_mid_layer_new), dim=0
            )
            ...
            other2_answerm_mid_layer = torch.cat(
                (other2_answerm_mid_layer, other2_answerm_mid_layer_new), dim=0
            )
            ...
            other1_entropies = torch.cat(
                (other1_entropies, other1_entropy_new), dim=0
            )
            other2_entropies = torch.cat(
                (other2_entropies, other2_entropy_new), dim=0
            )
    ...
    if phase == "validation":
        ask4conf_score = load_ask4conf_score(
            ...
            available_idxs=list(range(query_answer_last_token_last.shape[0])),
...
def get_index_of_valid_X(dataset_name, model_type, phase="test"):
    ...
    for d_idx, d in enumerate(data):
        ...
        if metric in mrouge[d_idx]:
            if phase != "test" or (
                SU_KEY in SU_data[d_idx]
            ):  # make sure SU has been calculated, and the metric has been calculated
...
def load_X_Y_with_phase(
    ...
    def get_file_name_list(layer_list, is_cross=False):
        ...
        if not is_cross:
            answerm_mid_layer_name = (
                "answerm_average_layer_" + str(layer_list[0]) + ".pt"
            )
            answerm_last_layer_name = (
                "answerm_average_layer_" + str(layer_list[1]) + ".pt"
            )
            answer_last1_token_mid_layer_name = (
                "answerm_last_1_token_layer_" + str(layer_list[0]) + ".pt"
            )
            answer_last1_token_last_layer_name = (
                "answerm_last_1_token_layer_" + str(layer_list[1]) + ".pt"
            )
            file_name_list = [
                query_average_mid_layer_name,
                query_average_last_layer_name,
            ...
            other_answerm_mid_layer_name = (
                "cross_average_layer_" + str(layer_list[0]) + ".pt"
            )
            other_answerm_last_layer_name = (
                "cross_average_layer_" + str(layer_list[1]) + ".pt"
            )
            other_answer_last1_token_mid_layer_name = (
                "cross_last_1_token_layer_" + str(layer_list[0]) + ".pt"
            )
            other_answer_last1_token_last_layer_name = (
                "cross_last_1_token_layer_" + str(layer_list[1]) + ".pt"
            )
            file_name_list = [
                query_average_mid_layer_name,
                query_average_last_layer_name,
    ...
    other1_answerm_entropies = torch.load(
        other1_output_dir + other_answerm_entropy_name
    )
    other1_query_probs = torch.load(other1_output_dir + query_probs_name)
    other1_answerm_probs = torch.load(
        other1_output_dir + other_answerm_probs_name
    )

    other2_query_entropies = torch.load(other2_output_dir + query_entropy_name)
    other2_answerm_entropies = torch.load(
        other2_output_dir + other_answerm_entropy_name
    )
    other2_query_probs = torch.load(other2_output_dir + query_probs_name)
    other2_answerm_probs = torch.load(
        other2_output_dir + other_answerm_probs_name
    )
...
def load_ask4conf_score(
    ...
    return df["prob"].iloc[
        [_ for _ in available_idxs if _ < df["prob"].index.stop]
    ]
