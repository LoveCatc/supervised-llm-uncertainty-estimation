...
parser.add_argument('--dataset_name', type=str, default="coqa", help='dataset name')
...
_,data_test_list,name_list,_,y_test,SU_test,ask4conf_test = load_X_Y(dataset_name,model_type,with_entropy=with_entropy)
...
for idx in range(len(y_pred_cal_logits_list)):
    ...
    nll_before,ece_before,brier_before = evaluate_calibration(y_pred_test_logits_list[idx],y_test.reshape(-1,1))
    if calibration_method == "TS":
        y_pred_test_TS_logits = TScalibrate(y_pred_cal_logits_list[idx],y_pred_test_logits_list[idx],y_cal.reshape(-1,1))
    elif calibration_method == "PS":
        y_pred_test_TS_logits = PScalibrate(y_pred_cal_logits_list[idx],y_pred_test_logits_list[idx],y_cal.reshape(-1,1))
    elif calibration_method == "HistBinning":
        y_pred_test_TS_logits = HistBinningCalibrate(y_pred_cal_list[idx],y_pred_test_list[idx],y_cal.reshape(-1,1),20)
    nll_after,ece_after,brier_after = evaluate_calibration(y_pred_test_TS_logits.reshape(-1,1),y_test.reshape(-1,1))
